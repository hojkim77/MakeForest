...
export function PostCard({ post, isLoggedIn, myReactionEmojis, feedFilters }: Props) {
  ...
  return (
    <article className="flex flex-col gap-md p-md bg-surface border-2 border-outline shadow-island">
      {/* Header */}
      <div className="flex items-center gap-sm">
        ...
        <div className="flex flex-col">
          ...
          {(post.dongName ?? post.user.dongCode) && (
            <span className="font-mono text-label text-on-surface-variant">{post.dongName ?? post.user.dongCode}</span>
          )}
        </div>
        <div className="ml-auto flex items-center gap-sm font-mono text-label text-on-surface-variant">
      ...
      {post.session?.todosPublic && post.session.todos.length > 0 && (
        <ul className="flex flex-col gap-xs pl-xs">
          {post.session.todos.map((todo, i) => (
            <li key={i} className={`font-mono text-label flex items-center gap-xs ${todo.done ? 'text-on-surface-variant line-through' : 'text-on-surface'}`}>
              <span>{todo.done ? '✓' : '○'}</span>
              <span>{todo.text}</span>
            </li>
          ))}
