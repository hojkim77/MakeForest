...
describe('pickDailyCreature — 지역코드+날짜 기반 결정론적 생명체 선택', () => {
  it('같은 입력 → 같은 결과 (결정론적)', () => {
    expect(pickDailyCreature('11', '2025-05-16'))
      .toBe(pickDailyCreature('11', '2025-05-16'));
  });
  ...
  it('서로 다른 지역코드가 서로 다른 생명체를 반환할 수 있음', () => {
    const codes = ['11', '26', '41:수원시', '41:고양시', '28'];
    const results = codes.map((rc) => pickDailyCreature(rc, '2025-05-16'));
  ...
  it('서로 다른 날짜가 서로 다른 생명체를 반환할 수 있음', () => {
    ...
    const results = dates.map((d) => pickDailyCreature('11', d));
    const unique = new Set(results);
    expect(unique.size).toBeGreaterThanOrEqual(2);
  });
});
